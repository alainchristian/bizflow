...
**Status:** Step 6 — Catalog (Products & Services) complete: single `catalog_items` table (`type` enum `product`/`service`, per the blueprint's merged-schema decision) rather than separate products/services tables, tenant-scoped + RLS on the Step 3/5 pattern. Money is `bigint` minor units + `char(3)` currency code (`common/database/money.transformer.ts` converts the bigint↔string boundary so callers just use a `number`) — this transformer is intended for reuse by every future money column (quotations, invoicing, payments), not a catalog-only concern. `sales.catalog_items.{view,create,manage}` permissions: view is open to all roles (a Member needs to read prices to build a future quotation), create/manage are owner/admin-only (unlike CRM, catalog pricing is administrative, not day-to-day data entry) — this is a deliberate, documented deviation from Step 5's "CRM is open to everyone" default, not an oversight. `tax_rule_id` is a nullable placeholder column for Step 7; no tax logic exists yet. Frontend reuses `DataTable` as-is (read-only rows, per the Step 5 design note) plus a create/edit form; price is entered/displayed as a decimal string and converted to/from integer minor units at the form boundary (`features/catalog/schemas.ts`). Also added `frontend/src/lib/zod-helpers.ts` (`optionalText`/`optionalEmail`) — a shared empty-string→undefined transform for optional form fields, replacing the field-by-field fix from Step 5's browser-testing bug, now applied across CRM and catalog forms.
**Next step:** Step 7 — Tax Engine (basic). See `/docs/roadmap/build-order.md`.
