...
**Status:** Step 7 — Tax Engine (basic) complete: `tax_rules` table (name, `rate_basis_points` integer, `is_inclusive`, `is_active`), tenant-scoped + RLS on the established pattern, plus a pure, dependency-free `TaxCalculationService` (`backend/src/sales/tax-calculation.service.ts`) handling exclusive rates (summed independently, not compounded), inclusive rates (backed out of the price, proportionally distributed with the last rule absorbing any rounding remainder so the breakdown always reconciles exactly), and mixed inclusive+exclusive on the same line. Rates are integer basis points, never floats, for the same reason money is integer minor units. Full design rationale — especially the rounding/reconciliation rules and the snapshot-don't-recompute guidance for Step 8/9 — is in `/docs/architecture/tax-engine.md`; read it before wiring this into quotations or invoicing. `catalog_items.tax_rule_id` (a placeholder since Step 6) now has a real FK and is validated tenant-scoped on create/update. Permissions: `sales.tax_rules.{view,create,manage}` are owner/admin-only **including view** — stricter than catalog, since a Member never needs to browse tax configuration to do their job. Frontend: a tax settings screen (reusing `DataTable`) and a "Tax rule" selector added to the catalog item form.
**Next step:** Step 8 — Quotations. See `/docs/roadmap/build-order.md`.
